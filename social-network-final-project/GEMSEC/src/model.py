...
class GEMSECWithRegularization(Model):
    ...
    def build(self):
        ...
        with self.computation_graph.as_default():
            ...
            self.gamma = tf.compat.v1.placeholder("float")
            ...
            self.learning_rate_new = tf.compat.v1.train.polynomial_decay(self.args.initial_learning_rate,
                                                               self.batch,
                                                               self.true_step_size,
                                                               self.args.minimal_learning_rate,
                                                               self.args.annealing_factor)

            self.train_op = tf.compat.v1.train.AdamOptimizer(self.learning_rate_new).minimize(self.loss,
