...
class DeepWalker:
    ...
    def __init__(self, args, vocab_size, degrees):
        ...
        self.train_labels = tf.compat.v1.placeholder(tf.int64, shape=[None, self.args.window_size])

        self.train_inputs = tf.compat.v1.placeholder(tf.int64, shape=[None])

        self.embedding_matrix = tf.Variable(tf.random.uniform([self.vocab_size, self.args.dimensions],
                                                              -0.1/self.args.dimensions,
                                                              0.1/self.args.dimensions))


        self.nce_weights = tf.Variable(tf.random.truncated_normal([self.vocab_size, self.args.dimensions],
                                       stddev=1.0/math.sqrt(self.args.dimensions)))

        self.nce_biases = tf.Variable(tf.random.uniform([self.vocab_size],
                                                        -0.1/self.args.dimensions,
                                                        0.1/self.args.dimensions))
...
class Clustering:
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.cluster_means = tf.Variable(tf.random.uniform([self.args.cluster_number, self.args.dimensions],
                                         -0.1/self.args.dimensions,
                                         0.1/self.args.dimensions))
...
class Regularization:
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.edge_indices_right = tf.compat.v1.placeholder(tf.int64, shape=[None])
        self.edge_indices_left = tf.compat.v1.placeholder(tf.int64, shape=[None])
        self.overlap = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
