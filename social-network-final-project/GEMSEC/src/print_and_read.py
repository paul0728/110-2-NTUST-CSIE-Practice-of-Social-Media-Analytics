...
def graph_reader(input_path):
    ...
    with open('GEMSEC_nodes.pickle', 'wb') as f:
        GEMSEC_nodes = pickle.dump(list(graph.nodes),f)

    '''
    X_train_pos,X_test_pos = train_test_split(edges.values.tolist(),test_size=0.2, random_state=9)
    train_graph_w_all_node=nx.from_edgelist(X_train_pos)
    train_graph_w_all_node.add_nodes_from(graph)
    return train_graph_w_all_node
    '''
    return graph
